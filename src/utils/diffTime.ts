...
const getDiffTime = (startTime: string, endTime: string, unit: string) => {
    // 判断当前月天数
    ...
    if (unit === 'd') {
        const d = diffValue / 1000 / 60 / 60 / 24;
        return d;
    }
    if (unit === 'h') {
        const h = diffValue / 1000 / 60 / 60;
        return h;
    }
    if (unit === 'm') {
        const m = diffValue / 1000 / 60;
        return m;
    }
    if (unit === 's') {
        const s = diffValue / 1000;
        return s;
    }
};
